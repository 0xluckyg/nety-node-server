...
function getChatrooms(socket) {
    socket.on('/self/getChatrooms', () => {
        User.findById(
            {_id: socket.userId}, 
            {chatrooms: 1, blocked: 1, contacts: 1}
        ).then(user => {
            if (!user) {
            ...
            return Chatroom.find({
                ...
                if (chatrooms) {
                    return returnChatrooms(chatrooms, user.contacts);
                }
            });
    ...
    function returnChatrooms(chatrooms, contacts) {        
        const fromIds = [];        
        const unreads = [];
        const isContact = [];
        chatrooms.forEach(chatroom => {           
            ...
            for (let i = 0; i < contacts.length; i++) {
                if (otherUser === contacts[i].toString()) {
                    isContact.splice(isContact.length-1, 1, true);                    
                    break;
                }
            }                        
        });                    
...
function sendMessage(socket, io) {
    ...
    function saveMessage(msg) {
        ...
        newMessage.save().then(savedMsg => {
            const emitMessage = savedMsg.toObject();
            emitMessage.name = msg.name;            
            socket.emit('/self/sendMessage/success', emitMessage);
            io.to(`${msg.toId}`).emit('/user/message', emitMessage);
        });
    }
}
